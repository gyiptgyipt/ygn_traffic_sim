...
x = {'right':[0,0,0], 'down':[320,360,306], 'left':[1400,1400,1400], 'up':[1030,1000,1060]}    
...
mid = {'right': {'x':220, 'y':640}, 'down': {'x':350, 'y':600}, 'left': {'x':1150, 'y':306}, 'up': {'x':560, 'y':200}}  # turning points 
rotationAngle = 3
...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        # Adjust coordinates based on the vehicle's direction and lane
        if(direction == 'right'):
        ...
        # Similar code for other directions ('left', 'down', 'up') ...
    ...
    def move(self):
        if(self.direction == 'right'):
            # Right direction logic (as you provided)
            ...
            if(self.willTurn == 1):
                if(self.crossed == 0 or self.x + self.currentImage.get_rect().width < mid[self.direction]['x']):
                    ...
                    (self.index == 0 or self.x + self.currentImage.get_rect().width < (vehicles[self.direction][self.lane][self.index-1].x - gap2) or vehicles[self.direction][self.lane][self.index-1].turned == 1)):
                        self.x += self.speed
                else:
                    if(self.turned == 0):
        ...
        # Movement logic for 'left' direction
            ...
            if(self.willTurn == 1):
                if(self.crossed == 0 or self.x > mid[self.direction]['x']):
                    ...
                    (self.index == 0 or self.x > (vehicles[self.direction][self.lane][self.index-1].x + vehicles[self.direction][self.lane][self.index-1].currentImage.get_rect().width + gap2) or vehicles[self.direction][self.lane][self.index-1].turned == 1)):
                        self.x -= self.speed
                else:
                    if(self.turned == 0):
                        self.rotateAngle += rotationAngle
        ...
        # Movement logic for 'down' direction
            ...
            if(self.willTurn == 1):
                if(self.crossed == 0 or self.y + self.currentImage.get_rect().height < mid[self.direction]['y']):
                    ...
                    (self.index == 0 or self.y + self.currentImage.get_rect().height < (vehicles[self.direction][self.lane][self.index-1].y - gap2) or vehicles[self.direction][self.lane][self.index-1].turned == 1)):
                        self.y += self.speed
                else:
                    if(self.turned == 0):
                        self.rotateAngle += rotationAngle
        ...
        # Movement logic for 'up' direction
            ...
            if(self.willTurn == 1):
                if(self.crossed == 0 or self.y > mid[self.direction]['y']):
                    ...
                    (self.index == 0 or self.y > (vehicles[self.direction][self.lane][self.index-1].y + vehicles[self.direction][self.lane][self.index-1].currentImage.get_rect().height + gap2) or vehicles[self.direction][self.lane][self.index-1].turned == 1)):
                        self.y -= self.speed
                else:
                    if(self.turned == 0):
                        self.rotateAngle += rotationAngle
...
# Define base green time and other vehicle times
baseGreenTime = 20  # Starting green light duration (in seconds)
carTime = 2  # Time taken for a car to pass the intersection (in seconds)
bikeTime = 1  # Time taken for a bike to pass the intersection (in seconds)
busTime = 3  # Time taken for a bus to pass the intersection (in seconds)
rickshawTime = 2.5  # Time taken for a rickshaw to pass the intersection (in seconds)
...
def setTime():
    ...
    for lane in range(3):  # loop through all lanes in current direction
        for vehicle in vehicles[directionNumbers[nextGreen]][lane]:
            if vehicle.crossed == 0:  # Vehicle is still at the stop line
    ...
    # Calculate the green time based on vehicle count
    greenTime = baseGreenTime + (noOfCars * carTime) + (noOfBikes * bikeTime) + (noOfBuses * busTime) + (noOfRickshaws * rickshawTime)
    
    # Apply minimum and maximum limits to green time
    ...
    print(f"Green Time for Signal {currentGreen}: {greenTime}")
...
class Main:
    ...
    background = pygame.image.load('./images/ygn.png')
